                ...
                alignment: MainAxisAlignment.spaceBetween,
                  children: [
                    ...
                    ElevatedButton(onPressed: (){
                      if(cart.checkInCart(catalog!)){
                        context.read<CartChangeNotifier>().removeItemFromCart(catalog!);
                      }else{
                    context.read<CartChangeNotifier>().addItemInCart(catalog!);
                      }
                    },
                    style: ButtonStyle(backgroundColor: MaterialStateProperty.all(MyTheme.bluishColor),
                    shape: MaterialStateProperty.all(StadiumBorder(),
                    )),
                      child:cart.checkInCart(catalog!) ?Icon(CupertinoIcons.cart_fill_badge_minus) :Icon(CupertinoIcons.cart_fill_badge_plus)
                      ).wh(80, 45),
